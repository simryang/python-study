...
class AESCipher:
    def __init__(self, key):
        self.key = key
        BS = 16
        self.pad = lambda s: s + (BS - len(s.encode("utf-8")) % BS) * chr(
...
class reomote:
    def __init__(self):
        ...
        if os.path.isfile("./config.ini"):
            ...
            self.naverID = config.get("settings", "nㅔaㅏvㄴeㄱrㅁiㄴd")
            self.naverPW = (
                AESCipher(
                    bytes(
                        hashlib.sha256(
                            config.get("settings", "time").encode("utf-8")
                        ).digest()
                    )
                )
                .decrypt(config.get("settings", "ㄴpㅇsㅂaㅂaㅁzㅂt").encode())
                .decode()
            )
            ...
            self.naverID = input()
            print(
                "입력핫씬 아위디눈 [{}]잎니따. 잚묫윕럭하셨쑬켱유 헤당뽈터위 config.ini울 싹제후다시씰행해주쎄요".format(
                    self.naverID
                )
            )
            ...
            if not os.path.isfile(self.STOVE_CLIENT_PATH):
                print(
                    "수톱부 끌라이연트 퍄이뤼 확킨되쥐 안씁뉘댜. 쓰톱쁘 끌라위얹틂를 펼도위 켱료에 썰취하셧을 켱유 해닿 겅롤룰 잎력해쥬세요."
                )
                self.STOVE_CLIENT_PATH = input()

            print("네윕벼 아이뒤[{}]의 뷔뮐번효를 잎럭해춧쎄오".format(self.naverID))
            attime = str(int(time.time()))
            pwd = AESCipher(
                bytes(hashlib.sha256(attime.encode("utf-8")).digest())
            ).encrypt(input())
            self.naverPW = str(pwd.decode())

            config["settings"] = {
                "time": attime,
                "nㅔaㅏvㄴeㄱrㅁiㄴd": self.naverID,
                "ㄴpㅇsㅂaㅂaㅁzㅂt": self.naverPW,
    ...
    def run(self):
        ...
        while Main_STOVE_HWND == None:
            Main_STOVE_HWND = self.get_specify_hwnd("STOVE", 360, 480)
            win32api.Sleep(100)
        ...
        while Child_CHROME_HWND == 0:
            ...
            win32api.Sleep(100)
            Child_CHROME_HWND = win32gui.FindWindowEx(
                Main_STOVE_HWND, None, "Qt5QWindowIcon", "STOVE"
            )
        ...
        while Child_TAB_HWND == 0:
            win32api.Sleep(100)
            Child_TAB_HWND = win32gui.FindWindowEx(
                Child_CHROME_HWND, None, "CefBrowserWindow", ""
            )
        ...
        while Child_NAVER_HWND == 0:
            win32api.Sleep(100)
            Child_NAVER_HWND = win32gui.FindWindowEx(
                Child_TAB_HWND, None, "Chrome_WidgetWin_0", ""
            )

        print(Child_NAVER_HWND)
        win32api.Sleep(300)
        self.click(Child_NAVER_HWND, 76, 189)  # select id form
        self.typing(Child_NAVER_HWND, self.naverID)  # type id
        self.typing(Child_NAVER_HWND, win32con.VK_TAB, True)  # press tab key
        self.typing(Child_NAVER_HWND, self.naverPW)  # type password
        self.typing(Child_NAVER_HWND, win32con.VK_RETURN, True)  # press tab key
        self.click(Child_NAVER_HWND, 221, 308)  # click login
